...
c_vec     = linspace(0.01, 200, Nc);
sigma_vec = linspace(0.25, 5, Ns);
...
parfor is = 1:Ns
    ...
    ell2 = 4*beta*xop^2 - lambda2*sigma;

    nu = 1.1*sigma*(lambdan-lambda2)/2/sqrt(abs(ell2));


    % Theorem-built P0(sigma)
    [P0_raw, lambda_x_raw] = build_P0(beta, xop, sigma, nu, lambda2, lambdan);
...
plot(c_th_curve(c_th_curve<max(c_vec)), sigma_vec(c_th_curve<max(c_vec)), 'w', 'LineWidth', 2);
hold on;
...
function [P0, lambda_x] = build_P0(beta, xop, sigma, nu, lambda2, lambdan)
